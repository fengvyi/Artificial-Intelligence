...
class AlphaBetaAdvanced {
     ...
     */
    static void run (Board.State player, Board board, int maxDepth) {
        ...
        /*
        if(board.getMoveCount() >= 5 && maxPly < maxDepth) {
        	maxPly++;
        	//System.out.println("Depth changes to " + maxPly);
        }
        
        if(board.getMoveCount() >= 20 && maxPly < maxDepth) {
        	maxPly++;
        	//System.out.println("Depth changes to " + maxPly);
        }
        
        if(board.getMoveCount() >= 30 && maxPly < maxDepth) {
        	maxPly++;
        	//System.out.println("Depth changes to " + maxPly);
        }
        */
    ...
    private static int getMax (Board.State player, Board board, double alpha, double beta, int currentPly) {
        ...
        if (indexOfBestMove != -1) {
            ...
            if(currentPly == 1) {
            	System.out.println("Player O moves at (" + indexOfBestMove / board.getBoardWidth() + "," + indexOfBestMove % board.getBoardWidth() + "), Total Move = " + board.getMoveCount());
    ...
    private static int getMin (Board.State player, Board board, double alpha, double beta, int currentPly) {
        ...
        if (indexOfBestMove != -1) {
            board.move(indexOfBestMove);
            if(currentPly == 1) {
            	System.out.println("Player X moves at (" + indexOfBestMove / board.getBoardWidth() + "," + indexOfBestMove % board.getBoardWidth() + "), Total Move = " + board.getMoveCount());
            	board.setPreMove(indexOfBestMove);
            }
        }
        return (int)beta;
