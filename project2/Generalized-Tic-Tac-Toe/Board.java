...
public class Board {
    ...
    public boolean isUseless (int index) {
        ...
        if(checkMate && distance > 2) return true;
        
        
        // Check Manhattan distance with max limit 2*M
        if(moveCount < 20) {
        	if(moveCount < 5 && distance > 3) return true;
        	if(moveCount < 10 && distance > 5) return true;
        	if(moveCount < 20 && distance > 2 * M) return true;
        }
        ...
        for(int i = start_row; i <= end_row; i++) {
        	for(int j = start_col; j <= end_col; j++) {
        		...
        		if(count / moveCount > 1 / 3 || count > 0) return false;
        	}
        }
    ...
    public int getScoreX () {
    	scoreX = 0;
    	for(int i = 0; i < M; i++){
    		//if(i == M - 1) scoreX += winningWindowsX[i] * i * i + 10000000;
    ...
    public int getScoreO () {
    	scoreO = 0;
    	for(int i = 0; i < M; i++){
    		//if(i == M - 1) scoreO += winningWindowsO[i] * i * i + 10000000;;
    		scoreO += winningWindowsO[i] * i * i;
    	}
        return scoreO;
    }
    ...
    public void updateScoreWindow (int col, int row, State player) {
    	...
    	if(player == State.X) {
    		if(scoreWindow[M - 1] - this.winningWindowsX[M - 1] > 0) {
    			checkMate = true;
    		}
    		this.winningWindowsX = scoreWindow;
    	}
    	else {
    		if(scoreWindow[M - 1] - this.winningWindowsO[M - 1] > 0) {
    			checkMate = true;
    		}
    ...
    private void checkWin (int col, int row, State player) {
    	...
    	if(player == Board.State.X) {
    		if(winningWindowsX[M] > 0) {
    			winner = playersTurn;
                gameOver = true;
                return;
    		}
    	...
    	if(player == Board.State.O) {
    		if(winningWindowsO[M] > 0) {
    			winner = playersTurn;
                gameOver = true;
                return;
    		}
